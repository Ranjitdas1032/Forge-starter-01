...
function handle({ event, data }) {
  if (event === "snapshot") {
    ...
    const urls = parseInt(data.urls) || 0;
    $("meta").textContent = "· " + (data.site || ""); $("urls").textContent = urls + " URLs";
    log(`Loaded ${urls} URLs from ${data.site || "site"}`); $("tbody").innerHTML = "";
    totals = { High: 0, Medium: 0, Low: 0, total: 0 };
  } else if (event === "issue") { addIssue(data); log(`Found ${data.count || 0} × ${data.type || ""}`); }
  else if (event === "summary") { log(`Audit complete: ${(data.total_issues || 0)} issue types`); }
  else if (event === "fixes") { log(`Fixes ready: ${(data.titles || []).length} titles, ${(data.redirect_map || []).length} redirects`); }
  else if (event === "exported") { $("export").innerHTML = "<b>report.html written ✓</b><br><a href='../outputs/report.html' style='color:#3a9aff;font-size:12px;text-decoration:none'>Open report</a>"; }
...
es.onerror = (e) => { console.error("[SSE connection error]", e); $("meta").textContent = "· connection lost"; };
